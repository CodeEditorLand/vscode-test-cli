...
const args = yargs(process.argv)
	...
	.option("label", {
		...
		type: "array",
		description:
			"Specify the test configuration to run based on its label in configuration",
		group: vscodeSection,
	})
	.option("code-version", {
		type: "string",
	...
	.option("list-configuration", {
		type: "boolean",
		description:
			"List configurations and that they woud run, without executing them",
		group: rulesAndBehavior,
	})
	.option("fail-zero", {
		type: "boolean",
	...
	for (const { config, path, env, extensionTestsPath } of resolvedConfigs) {
		if (isDesktop(config)) {
			...
			const nextCode = await electron.runTests({
				...
				extensionDevelopmentPath:
					config.extensionDevelopmentPath?.slice() || dirname(path),
				extensionTestsPath,
				extensionTestsEnv: {
					...config.env,
					...env,
				...
				reuseMachineInstall:
					...
					"fromMachine" in config.useInstallation
						? config.useInstallation.fromMachine
						: undefined,
				vscodeExecutablePath:
					config.useInstallation &&
					"fromPath" in config.useInstallation
