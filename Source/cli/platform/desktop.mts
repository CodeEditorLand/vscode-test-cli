...
export class DesktopPlatform implements IPlatform {
	...
	public async prepare({
		...
		if (args.run?.length) {
			test.files = args.run.map((r) =>
				resolvePath(process.cwd(), String(r)),
			);
		}

		const preload = await Promise.all(
			[
				...ensureArray(test.mocha?.preload || []),
				...ensureArray(args.file || []),
			].map((p) => mustResolve(config.dir, String(p))),
		);
...
class PreparedDesktopRun implements IPreparedRun {
	...
	private async importTestElectron() {
		const electronPath = await mustResolve(
			this.config.dir,
			"@vscode/test-electron",
		);
		const electron: typeof Electron = await import(
	...
	public async run({ coverage }: IRunContext): Promise<number> {
		...
		try {
			...
			if (
				typeof e === "string" ||
				(electron.TestRunFailedError &&
					e instanceof electron.TestRunFailedError)
			) {
				return 1;
			}
	...
	private baseCliOptions() {
		return {
			...
			reuseMachineInstall:
				this.test.useInstallation &&
				"fromMachine" in this.test.useInstallation
					? this.test.useInstallation.fromMachine
					: undefined,
			vscodeExecutablePath:
				this.test.useInstallation &&
				"fromPath" in this.test.useInstallation
	...
	private async installDependentExtensions(electron: typeof Electron) {
		...
		if (
			!this.args.skipExtensionDependencies &&
			!this.test.skipExtensionDependencies
		) {
		...
		const [cli, ...cliArgs] =
			electron.resolveCliArgsFromVSCodeExecutablePath(vscodePath, opts);
		...
		return new Promise<void>((resolve, reject) => {
			...
			const installer = spawn(shell ? `"${cli}"` : cli, cliArgs, {
				stdio: "pipe",
				shell,
			});
			let output: string = "";
				...
				if (e !== 0) {
					reject(
						new CliExpectedError(
							`Failed to install extensions (${exts}): ${output}`,
						),
					);
				} else {
					resolve();
