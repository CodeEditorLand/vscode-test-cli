...
export async function tryLoadConfigFile(
	path: string,
): Promise<ResolvedTestConfiguration> {
	...
	try {
		...
		return await ResolvedTestConfiguration.load(
			{ tests: ensureArray(loaded) },
			path,
		);
	} catch (e) {
		throw new CliExpectedError(
...
export class ResolvedTestConfiguration
	implements IConfigurationWithGlobalOptions
{
		...
		for (const test of config.tests) {
			if (test.mocha?.require) {
				test.mocha.require = await Promise.all(
					ensureArray(test.mocha.require).map((f) =>
						mustResolve(dir, f),
					),
				);
			}
	...
	public extensionDevelopmentPath(test: TestConfiguration) {
		return ensureArray(
			test.extensionDevelopmentPath?.slice() || this.dir,
		).map((p) => (isAbsolute(p) ? p : join(this.dir, p)));
	}
}
